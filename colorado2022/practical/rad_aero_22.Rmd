...
First we add the depolarization ratio (DR) as a parameter:
```{r, eval=SHOW_ANSWERS}
# add depolarization ratio (DR) as a parameter (following Kilambi 2018):
my_ppi <- calculate_param(my_ppi, DR = 10 * log10((ZDR + 1 - 2 * (ZDR^0.5) * RHOHV) /
  (ZDR + 1 + 2 * (ZDR^0.5) * RHOHV)))
```
Like correlation coefficient, it has a good (even better) ability to distinguish weather and biology:
```{r, eval=SHOW_ANSWERS}
...
```{r, echo=SHOW_ANSWERS, eval=SHOW_ANSWERS}
# Now let us screen out the reflectivity areas with DR < -10 dB:
my_ppi_clean <- calculate_param(my_ppi, DBZH = ifelse(DR < -10, NA, DBZH))
# plot the original and cleaned up reflectivity:
map(my_ppi, map = basemap, param = "DBZH", zlim = c(-20, 40))
map(my_ppi_clean, map = basemap, param = "DBZH", zlim = c(-20, 40))
```
## using MistNet:
```{r, eval=SHOW_ANSWERS}
# apply the MistNet model to the polar volume file:
...
my_vplist <- readRDS("data/KBRO20170514.rds")
...
load("data/KBGM20170527-20170602.RData")
...
**Exercise 13:** Visually compare the PPI for the 1.0 degree sweep and the vertically integrated PPI, and explain the differences. Hint: for the clearest comparison, make plots of comparable parameters that are either linear or logarithmic in bird density.
```{r, echo=SHOW_ANSWERS, eval=SHOW_ANSWERS,warning=FALSE}
# 
my_pvol %>%
  get_scan(elev = 1.0) %>%
