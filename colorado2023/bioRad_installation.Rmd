...
We recommend upgrading to a recent version of R (latest version is 4.3.1, found at https://www.r-project.org/) and using [Rstudio](https://www.rstudio.com/products/rstudio/) as a graphical user interface.
...
The bioRad installation of the previous step should have installed the **[vol2birdR](https://adriaandokter.com/vol2birdR/)** package, which is an important dependency package for bioRad. Next, we need to run a few vol2birdR commands to enable MistNet (expect a ~ 150 Mb download):
```{r, eval=FALSE}
...
### Step 2: download mistnet libraries and model
The bioRad installation of the previous step should have installed the **[vol2birdR](https://adriaandokter.com/vol2birdR/)**, which is an important dependency package for bioRad. Next, we need to run a few vol2birdR commands to install MistNet libraries (expect a ~ 150 Mb download)
```{r, eval=FALSE}
vol2birdR::install_mistnet()
```
Next, we download the MistNet model into vol2birdR (expect >500 Mb download, this step can take a while when you have a slow internet connection)
...
The bioRad installation of the previous step should have automatically installed the **[vol2birdR](https://adriaandokter.com/vol2birdR/)** package as well, which is an important dependency package for bioRad. Next, we need to run a few vol2birdR commands to install MistNet libraries (expect a ~ 150 Mb download)
```{r, eval=FALSE}
vol2birdR::install_mistnet()
```
Next, we download the mistnet model into vol2birdR (expect >500 Mb download, this step can take a while when you have a slow internet connection)
